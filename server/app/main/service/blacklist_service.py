...
def add_token_to_database(encoded_token):
    """
    Adds a new token to the database. It is not revoked when ti is added.
    :param encoded_token
...
def revoke_token(user_identity):
    ...
    """
    tokens = Blacklist.query.filter_by(user_identity=user_identity, revoked=False).all()
    for token in tokens:
        token.revoked = True
        db.session.commit()
...
def unrevoke_token(token_id, user):
    ...
    try:
        ...
        raise TokenNotFound("Could not find the token {}".format(token))
